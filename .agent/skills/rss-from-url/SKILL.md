...
```bash
python .agent/skills/rss-from-url/infer_rss_config.py "<URL>" [--output 文件名] [--title "标题"]
```
...
- **条目容器**：按优先级尝试 `article[class*="ArticleList"]` → `article` → `[class*="card"]` → `[class*="post"]` → `[class*="item"]` → `main ul li`，选能匹配多条且每条内能区分标题/链接的。
